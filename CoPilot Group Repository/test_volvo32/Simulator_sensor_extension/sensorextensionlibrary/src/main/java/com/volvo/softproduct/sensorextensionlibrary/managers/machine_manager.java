...
import com.volvo.softproduct.sensorextensionlibrary.services.*;
...
public class machine_manager {
    ...
    private long_value RestoreLPreferencese(int dataid)
    {
        ...
        SharedPreferences simulatorValuePrefs = _activityContext.getSharedPreferences("simulatorvalue", _activityContext.MODE_WORLD_READABLE);
        String prefData = simulatorValuePrefs.getString(DATA_STATE+dataid, "0");
        String prefTime = simulatorValuePrefs.getString(TIME_STATE+dataid, "-1");
    ...
    private float_value RestoreDPreferencese(int dataid)
    {
        long currenttime = System.currentTimeMillis();
        SharedPreferences simulatorValuePrefs = _activityContext.getSharedPreferences("simulatorvalue", _activityContext.MODE_WORLD_READABLE);
        String prefData = simulatorValuePrefs.getString(DATA_STATE+dataid, "0.0");
        String prefTime = simulatorValuePrefs.getString(TIME_STATE+dataid, "-1");
        long time = Long.parseLong(prefTime);
        value_status status = value_status.OK;
        if(time == -1)
